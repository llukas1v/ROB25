...
class SO2:
    ...
    def __init__(self, angle: float = 0.0) -> None:
        ...
        super().__init__()
        #OK todo HW01: implement computation of rotation matrix from the given angle
        C = np.cos(angle)
        S = np.sin(angle)
        self.rot = np.array([[C, -S],
                             [S,  C]])
    ...
    def __mul__(self, other: SO2) -> SO2:
        ...
        Comp_rot = SO2() 
        Comp_rot.rot = self.rot @ other.rot #in this case order is irrelevant 
        return Comp_rot

    @property
    def angle(self) -> float:
        """Return angle [rad] from the internal rotation matrix representation."""
        #OK todo: HW01: implement computation of the angle from rotation matrix self.rot.
        return np.arctan2(self.rot[1,0],self.rot[0,0]) #takes Sin ad Cos form rotation matrix self.rot. and copute angele by arctan goniometric fnc.
    ...
    def inverse(self) -> SO2:
        ...
        inv_matrix = self.rot.T          #numpy create a transpondet matrix
        new_inv_matrix = SO2()           #create a new SO2 object
        new_inv_matrix.rot = inv_matrix  #save inversion matrix
        return new_inv_matrix            #return new object 
        #pass
